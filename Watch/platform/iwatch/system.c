...
void system_restore()
{
	...
	if (globaldata.checksum == CRCINIRES && globaldata.now.year < 2050
			&& globaldata.now.month <= 12 && globaldata.now.day <= 31)
	{
...
void system_rebootToTest()
{
	...
	// Initiate Data
	new_data.system_reset = 0;
	new_data.system_debug = 1;
	new_data.system_testing = 1;
