...
{ -1, "Reboot Kreyos", &reboot_to_normal },
{ -1, "Test Mode", &reboot_to_test },
...
extern uint8_t recordoperation_process(uint8_t ev, uint16_t lparam,
		void* rparam);
...
static void drawMenuItem(tContext *pContext, const tFont* textFont,
		int MENU_SPACE, const struct MenuItem *item, int index, int selected)
{
	if (selected)
	{
		// draw a rect
	...
	tRectangle rect =
	{ 2, 23 + index * MENU_SPACE, LCD_WIDTH - 2, 15 + (index + 1) * MENU_SPACE };
	...
	if (item->icon < 0x80 && item->icon != 0)
	{
		...
		GrStringDraw(pContext, (const char*) &item->icon, 1, 4,
				13 + (MENU_SPACE - 16) / 2 + index * MENU_SPACE, 0);
	}

	GrContextFontSet(pContext, textFont);
	if (item->icon < 0x80 && item->icon != 0)
	{
		GrStringDraw(pContext, item->name, -1, 40,
				17 + (MENU_SPACE - 16) / 2 + index * MENU_SPACE, 0);
	}
	else
	{
		char buf[20];
		int width;
		// <= 0
		GrStringDraw(pContext, item->name, -1, 5,
				17 + (MENU_SPACE - 16) / 2 + index * MENU_SPACE, 0);
		...
		GrStringDraw(pContext, buf, -1, LCD_WIDTH - 12 - width,
				17 + (MENU_SPACE - 8) / 2 + index * MENU_SPACE, 0);
	}
}
...
static void OnDraw(tContext *pContext)
{
	...
	for (int i = 0; i < NUM_MENU_A_PAGE; i++)
	{
		...
		drawMenuItem(pContext, textfont, MAINMENU_SPACE, item, i,
				current == currentTop + i);
		item++;
	}
	...
	if (item->name != NULL)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			GrLineDrawH(pContext, LCD_WIDTH / 2 - i, LCD_WIDTH / 2 + i,
			LCD_Y_SIZE - 5 - i);
		}
		GrContextForegroundSet(pContext, ClrBlack);
	}
...
uint8_t about_process(uint8_t ev, uint16_t lparam, void* rparam)
{
	...
	case EVENT_WINDOW_PAINT:
	{
		...
		while (item != NULL)
		{
			...
			drawMenuItem(pContext, &g_sFontGothic18b, (168 - 30) / 5, item, i,
					0);
			i++;
			item++;
		}
